...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Specific Mobile Sales Method
    $m_method = $_POST['sales_method_mobile'] ?? 'Mobile Sales';
    $m_comment = $_POST['board_comment'] ?? '';
    $sales_method = ($m_method === 'Other' && $m_comment) ? "Mobile Sales ($m_comment)" : "Mobile Sales ($m_method)";
    ...
    if ($valid) {
        ...
        if ($check_stmt->fetch()) {
            $valid = false;
            $message = "<div style='display:flex; align-items:center; gap:10px;'><span style='font-size:1.5rem;'>⚠️</span> <div><strong>Duplicate Seller Code!</strong><br>A mobile seller with the code '<strong>$seller_code</strong>' already exists in the system.</div></div>";
            $status = 'error';
        }
    }
    ...
    if ($valid) {
        try {
            ...
            $reg_number = $agent_code . "-" . str_pad($count, 3, '0', STR_PAD_LEFT);
            ...
            $message = "
                <div class='success-badge'>
                    ...
                    <div style='font-weight: 700; font-size: 1.2rem; margin-bottom: 5px;'>Mobile Seller Registration Successful!</div>
                    ...
                    <div style='display: flex; gap: 10px; justify-content: center;'>
                        ...
                        <a href='mobile_seller.php' class='btn-action' style='background: rgba(255,255,255,0.1); color: #fff; text-decoration: none; padding: 8px 15px; border-radius: 8px; font-weight: 600;'>Add New</a>
                    </div>
                </div>";
            $status  = 'success';
            log_activity($pdo, "Added Mobile Seller", "Reg: $reg_number, Name: $seller_name", "seller");
    ...
    if ($existing) {
        ...
        try {
            $upd = $pdo->prepare("UPDATE counters SET
                ...
                nic_type=?, nic_old=?, nic_new=?, joined_year=?,
...
<head>
    ...
    <title>Mobile Seller Entry - NLB Seller Map Portal</title>
    ...
    <style>
        ...
        .approval-chip:has(input:checked) {
            background: rgba(0, 212, 255, 0.15);
            border-color: #00d4ff;
            color: #00d4ff;
            font-weight: 600;
        }
        .approval-chip input { display: none; }
        ...
        /* GN Multi-Select Grid */
        .gn-checkbox-container {
            margin-top: 1rem;
        ...
        .gn-grid::-webkit-scrollbar { width: 4px; }
        .gn-grid::-webkit-scrollbar-track { background: transparent; }
        .gn-grid::-webkit-scrollbar-thumb { background: var(--secondary-color); border-radius: 10px; }
        ...
        .gn-checkbox-item:hover { background: rgba(255, 255, 255, 0.08); }
        .gn-checkbox-item.selected {
            border-color: var(--secondary-color);
            background: rgba(0, 114, 255, 0.1);
        }
        .gn-checkbox-item label {
            font-size: 0.8rem;
            color: var(--text-main);
            margin: 0;
            cursor: pointer;
        ...
        .gn-checkbox-item input { width: 16px; height: 16px; accent-color: var(--secondary-color); cursor: pointer; }
        .gn-checkbox-item.hidden { display: none; }
    </style>
    <script>
...
<div class="container wide">
        ...
        <form method="POST" action="index.php?search_seller=<?php echo urlencode($edit_record['seller_code']); ?>" enctype="multipart/form-data">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem;">
                ...
                <div class="form-group" style="margin:0;">
                    ...
                    <select name="ie_joined_year" class="form-select-modern" style="padding:0.4rem;">
                        <option value="">-- Select Year --</option>
                        <?php
                        for($y = date("Y"); $y >= 1970; $y--) {
                            $sel = ($edit_record['joined_year'] == $y) ? 'selected' : '';
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem;">
                <div class="form-group" style="margin:0;">
                    ...
                    <input type="date" name="ie_birthday" value="<?php echo htmlspecialchars($edit_record['birthday'] ?? ''); ?>">
                </div>
                <div class="form-group" style="margin:0;">
                    <label>Mobile Sales Method</label>
                    <select name="ie_sales_method">
                        <option value="Bicycle" <?php echo $edit_record['sales_method'] === 'Bicycle' ? 'selected' : ''; ?>>Bicycle</option>
                        <option value="Motorcycle" <?php echo $edit_record['sales_method'] === 'Motorcycle' ? 'selected' : ''; ?>>Motorcycle</option>
                        <option value="Sales Board" <?php echo $edit_record['sales_method'] === 'Sales Board' ? 'selected' : ''; ?>>Sales Board</option>
                        <option value="Other" <?php echo $edit_record['sales_method'] === 'Other' ? 'selected' : ''; ?>>Other</option>
                    </select>
                </div>
            </div>
            ...
            <div style="margin-bottom:1rem;">
                <div class="form-group">
                    ...
                    <textarea name="ie_address" rows="2"><?php echo htmlspecialchars($edit_record['address'] ?? ''); ?></textarea>
                </div>
            </div>
            ...
            <div style="margin-bottom:1rem;">
                <div class="form-group">
                    ...
                    <textarea name="ie_address2" rows="2"><?php echo htmlspecialchars($edit_record['address2'] ?? ''); ?></textarea>
                </div>
            </div>
    ...
    <div class="entry-layout">
        ...
        <div class="form-panel">
            <h2 style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                <span>🚶 Mobile Seller Details Entry</span>
            ...
            <form action="" method="POST" enctype="multipart/form-data" id="entryForm">
                ...
                $exclude_labels = ['Counter Status', 'Counter Sticker Quality', 'Repair States'];
                $main_fields = array_filter($custom_fields, fn($f) => 
                    ($f['display_section'] ?? 'additional') === 'main' && 
                    !in_array($f['field_label'], $exclude_labels)
                );
                $additional_fields = array_filter($custom_fields, fn($f) => 
                    ($f['display_section'] ?? 'additional') === 'additional' && 
                    !in_array($f['field_label'], $exclude_labels)
                );
                ?>
                ...
                <div class="field-row">
                    ...
                    <div class="form-group">
                        ...
                        <select id="joined_year" name="joined_year" onchange="updatePreview()" class="form-select-modern">
                            <option value="">-- Select Year --</option>
                            <?php 
                            $currentYear = date("Y");
                ...
                <div class="field-row">
                    <div class="form-group">
                        ...
                        <input type="date" id="birthday" name="birthday" required oninput="updatePreview()" style="width: 100%; padding: 0.75rem 1rem; background: var(--nav-bg); border: 2px solid var(--glass-border); border-radius: 12px; color: var(--text-main); font-weight: 600; font-family: 'Outfit', sans-serif;">
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    <label for="address">🏠 Seller Address (ලිපිනය) *</label>
                    <textarea id="address" name="address" required placeholder="Enter Full Seller Address" oninput="updatePreview()"></textarea>
                </div>

                <div class="form-group" style="margin-bottom: 1.5rem;">
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:6px;">
                        <label for="address2" style="margin:0;">🏡 Home Address (නිවාස ලිපිනය)
                            <span style="font-size:0.7rem; color:var(--text-muted); font-weight:400; margin-left:6px;">(අත්‍යාවශ්‍ය නොවේ)</span>
                        </label>
                        <button type="button" id="toggle_address2_btn" onclick="toggleAddress2()" style="font-size:0.72rem; background:rgba(0,212,255,0.1); border:1px solid rgba(0,212,255,0.25); color:#00d4ff; border-radius:8px; padding:3px 10px; cursor:pointer; font-weight:600;">+ Add Home Address</button>
                    </div>
                    <div id="address2_wrap" style="display:none;">
                ...
                <div style="background: rgba(0,212,255,0.03); padding: 1.5rem; border-radius: 18px; border: 1px solid rgba(0,212,255,0.1); margin-bottom: 2rem;">
                    <h3 style="font-size: 0.95rem; color: #00d4ff; margin-bottom: 1.25rem; display: flex; align-items: center; gap: 8px;">
                        <span>📍</span> Location Status (ස්ථානීය තොරතුරු)
                    </h3>
                    ...
                    <div class="field-row">
                        <div class="form-group">
                            ...
                            <select id="province" name="province" required onchange="loadDistricts(); updatePreview()" class="form-select-modern">
                                <option value="">-- Select Province --</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="district">District Name (දිස්ත්‍රික්කය)</label>
                            <select id="district" name="district" required onchange="loadDSDivisions(); updatePreview()" disabled class="form-select-modern">
                    ...
                    <div class="field-row">
                        <div class="form-group">
                            ...
                            <select id="ds_division" name="ds_division" required onchange="loadGNDivisions(); updatePreview()" disabled class="form-select-modern">
                    ...
                    <div style="margin-top: 1.5rem;">
                        <div class="form-group">
                            <label>GN Division Name (ග්‍රාම නිලධාරී කොට්ඨාසය) *</label>
                            <p style="font-size: 0.7rem; color: var(--text-muted); margin-bottom: 8px;">(කොට්ඨාස එකක් හෝ කිහිපයක් තෝරාගත හැක / You can select one or more divisions)</p>
                            
                            <!-- Search and Checkbox Grid -->
                            <div id="gn_checkbox_container" class="gn-checkbox-container" style="opacity: 0.5; pointer-events: none;">
                                <div class="gn-search-box">
                                    ...
                                    <input type="text" id="gn_search" placeholder="Search GN Division..." oninput="filterGNDivisions()" style="padding-left: 2.2rem;">
                                </div>
                                <div id="gn_grid" class="gn-grid">
                                    <p style="color: var(--text-muted); font-size: 0.8rem; padding: 10px;">Please select DS Division first.</p>
                                </div>
                            </div>

                            <!-- Hidden Multiple Select for Submission -->
                            <select id="gn_division" name="gn_division[]" multiple required style="display: none;"></select>
                    ...
                    <div class="form-group" style="margin-top: 1.5rem; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 1.5rem;">
                        ...
                        <div style="display: flex; gap: 10px;">
                            <input type="text" id="location_link" name="location_link" placeholder="Paste Google Maps link here" oninput="updatePreview()" style="flex: 1;">
                            <button type="button" onclick="getLocationLink()" class="btn-gps" style="background: rgba(0, 212, 255, 0.1); border: 1px solid rgba(0,212,255,0.25); color: #00d4ff; border-radius: 12px; padding: 0.6rem 1.25rem; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; white-space: nowrap;">📍 Get GPS</button>
                        </div>
                        <p id="location_status" style="font-size: 0.72rem; color: var(--text-muted); margin-top: 6px;">Use current GPS location or paste map link.</p>
                    </div>
                    <?php endif; ?>
                </div>

                <?php if (!empty($main_fields)): ?>
                <div style="margin-top:0.5rem; padding-top:1rem; border-top:1px solid var(--glass-border); margin-bottom: 1.5rem;">
                    <?php render_cf_group($main_fields); ?>
                </div>
                <?php endif; ?>

                <div class="form-group" style="margin-bottom: 1.5rem;">
                    <label>🚲 Sales Method (විකුණුම් ක්‍රමය)</label>
                    <div class="radio-group" style="margin-top: 5px;">
                        <input type="radio" id="sm_bicycle" name="sales_method_mobile" value="Bicycle" checked onchange="toggleMobileOther(); updatePreview()">
                        <label for="sm_bicycle" class="radio-pill"><span>Bicycle</span></label>
                        
                        <input type="radio" id="sm_motorcycle" name="sales_method_mobile" value="Motorcycle" onchange="toggleMobileOther(); updatePreview()">
                        <label for="sm_motorcycle" class="radio-pill"><span>Motorcycle</span></label>
                        
                        <input type="radio" id="sm_board" name="sales_method_mobile" value="Sales Board" onchange="toggleMobileOther(); updatePreview()">
                        <label for="sm_board" class="radio-pill"><span>Sales Board</span></label>
                        
                        <input type="radio" id="sm_other" name="sales_method_mobile" value="Other" onchange="toggleMobileOther(); updatePreview()">
                        <label for="sm_other" class="radio-pill"><span>Other</span></label>
                    </div>
                </div>

                <div id="mobile_other_wrap" class="form-group" style="display:none; margin-bottom: 1.5rem;">
                    <label for="board_comment">📝 Other Method Details</label>
                    <input type="text" id="board_comment" name="board_comment" placeholder="Specify other sales method" oninput="updatePreview()">
                </div>
                ...
                <!-- Location link removed for mobile sales -->
                ...
                <div style="margin-top: 1.5rem; display: flex; flex-direction: column; gap: 1rem;">
                    ...
                    <button type="button" onclick="submitAsDraft()" class="btn-submit" style="background: rgba(251, 191, 36, 0.1); color: #fbbf24; border: 1px solid rgba(251, 191, 36, 0.3); display: flex; align-items: center; justify-content: center; gap: 10px;">
                        📍 Pin Location Only (Seller Unavailable)
                    </button>
                    <p style="font-size: 0.7rem; color: var(--text-muted); text-align: center; margin-top: -5px;">Use this if the seller is closed or not available. You can fill other details later from the dashboard.</p>
                </div>
            </form>
        </div>
    ...
    function getLocationLink() {
        const ls = document.getElementById('location_status');
        const li = document.getElementById('location_link');
        if(!navigator.geolocation) { ls.textContent="Not supported"; return; }
        ls.textContent="Locating...";
        navigator.geolocation.getCurrentPosition(p=>{
            const lat=p.coords.latitude, lng=p.coords.longitude;
            li.value=`https://www.google.com/maps?q=${lat},${lng}`;
            ls.textContent="Captured!"; ls.style.color="#4ade80";
        }, e=>{ ls.textContent="Error: "+e.message; ls.style.color="#f87171"; });
    }
    ...
    function submitAsDraft() {
        ...
        if (!confirm('This will save only the location and basic info. Continue?')) return;

        document.getElementById('is_draft').value = '1';
        const form = document.getElementById('entryForm');
        const nameInput = document.getElementById('seller_name');
        if (nameInput && !nameInput.value) nameInput.value = 'Draft (Location Pinned)';
        
        const sellerCode = document.getElementById('seller_code');
        if (sellerCode && !sellerCode.value) sellerCode.value = 'DRAFT-' + Date.now();

        const address = document.getElementById('address');
        if (address && !address.value) address.value = 'Pinned Location - Details Pending';
        ...
        form.submit();
                    ...
                    <div class="preview-row">
                        <span class="preview-label">Joined Year:</span>
                        <span class="preview-val" id="prev_year">-</span>
                    </div>
                    <div class="preview-row">
                    ...
                    <?php if ($enable_location): ?>
                    <div class="preview-section-title">📍 Location Status</div>
                    <div class="preview-row">
                    ...
                    <div class="preview-row">
                        ...
                        <span class="preview-val" id="prev_ds">-</span>
                    </div>
                    <div class="preview-row" style="flex-direction: column; align-items: flex-start;">
                        <span class="preview-label">GN Div(s):</span>
                        <span class="preview-val" id="prev_gn" style="text-align: left; width: 100%; margin-top: 4px;">-</span>
                    </div>
                    <?php endif; ?>
                    <div class="preview-row">
                    ...
                    <div style="margin-top: 10px; display: flex; justify-content: center;">
                        <div style="text-align:center; width: 120px;"><small>Seller Photo</small><img id="thumb_seller" style="width: 100%; height: 120px; object-fit: cover; border-radius: 8px; display: none; border: 1px solid var(--glass-border);"></div>
                    </div>
                </div>
...
<script>
    function toggleMobileOther() {
        const otherSelected = document.getElementById('sm_other').checked;
        const wrap = document.getElementById('mobile_other_wrap');
        wrap.style.display = otherSelected ? 'block' : 'none';
        if (otherSelected) document.getElementById('board_comment').focus();
    }

    function toggleAddress2() {
        const wrap = document.getElementById('address2_wrap');
        const btn = document.getElementById('toggle_address2_btn');
        if (wrap.style.display === 'none') {
            wrap.style.display = 'block';
            btn.innerText = '− Remove Home Address';
            btn.style.background = 'rgba(248,113,113,0.1)';
            btn.style.borderColor = 'rgba(248,113,113,0.25)';
            btn.style.color = '#f87171';
        } else {
            wrap.style.display = 'none';
            document.getElementById('address2').value = '';
            btn.innerText = '+ Add Home Address';
    ...
    function updatePreview() {
        ...
        set('prev_year', document.getElementById('joined_year').value);
        set('prev_birthday', document.getElementById('birthday').value);
          ...
          set('prev_ds',       getSelectedText('ds_division'));
          
          // Handle multiple GN divisions
          const gnSelect = document.getElementById('gn_division');
          const gnTexts = gnSelect ? Array.from(gnSelect.selectedOptions).map(o => o.text) : [];
          const gnVal = gnTexts.join(', ');
          document.getElementById('prev_gn').innerText = gnVal || '-';
          if (gnVal) document.getElementById('prev_gn').classList.remove('empty');
          else document.getElementById('prev_gn').classList.add('empty');
          <?php endif; ?>
        ...
        // Method
        const methodEl = document.querySelector('input[name="sales_method_mobile"]:checked');
        const methodComment = document.getElementById('board_comment').value;
        let methodText = methodEl ? methodEl.value : 'Mobile Sales';
        if (methodEl && methodEl.value === 'Other' && methodComment) {
            methodText = 'Mobile Sales (' + methodComment + ')';
        } else {
            methodText = 'Mobile Sales (' + methodText + ')';
        }
        document.getElementById('prev_method').textContent = methodText;
    ...
    function shareWhatsApp() {
        ...
        const text = `*New Mobile Seller Registration*\n\n` +
                     `*Dealer:* ${d_code}\n` +
                     `*Agent:* ${a_code}\n` +
                     `*Seller Code:* ${s_code}\n` +
                     `*Name:* ${s_name}\n` +
    ...
    window.addEventListener('DOMContentLoaded', () => {
        fetch('data/data.json?v=<?php echo @filemtime("data/data.json") ?: time(); ?>')
            .then(res => res.json())
            .then(data => {
                locationData = data;
                populateProvinces();
    ...
    function loadGNDivisions() {
        ...
        const gnGrid = document.getElementById('gn_grid');
        const gnWrapper = document.getElementById('gn_checkbox_container');
        
        gnGrid.innerHTML = '';
        gnWrapper.style.opacity = '0.5';
        gnWrapper.style.pointerEvents = 'none';
        ...
        if (gnDivs.length === 0) {
            gnGrid.innerHTML = '<p style="color: var(--text-muted); font-size: 0.8rem; padding: 10px;">No GN Divisions found.</p>';
            return;
        }

        gnDivs.forEach(gn => {
            const item = document.createElement('div');
            item.className = 'gn-checkbox-item';
            item.innerHTML = `
                <input type="checkbox" id="gn_${gn.replace(/[^a-z0-9]/gi, '_')}" value="${gn}" onchange="toggleGNSelection(this)">
                <label for="gn_${gn.replace(/[^a-z0-9]/gi, '_')}">${gn}</label>
            `;
            gnGrid.appendChild(item);
        });

        gnWrapper.style.opacity = '1';
        gnWrapper.style.pointerEvents = 'auto';
    }

    function clearGNGrid() {
        const gnGrid = document.getElementById('gn_grid');
        const gnWrapper = document.getElementById('gn_checkbox_container');
        gnGrid.innerHTML = '<p style="color: var(--text-muted); font-size: 0.8rem; padding: 10px;">Please select DS Division first.</p>';
        gnWrapper.style.opacity = '0.5';
        gnWrapper.style.pointerEvents = 'none';
        syncGNHiddenSelect();
    }

    function toggleGNSelection(checkbox) {
        const item = checkbox.closest('.gn-checkbox-item');
    ...
    function syncGNHiddenSelect() {
        const hiddenSelect = document.getElementById('gn_division');
        const checked = document.querySelectorAll('#gn_grid input[type="checkbox"]:checked');
    ...
    function filterGNDivisions() {
        const query = document.getElementById('gn_search').value.toLowerCase();
        const items = document.querySelectorAll('.gn-checkbox-item');
        items.forEach(item => {
            const text = item.querySelector('label').innerText.toLowerCase();
            if (text.includes(query)) item.classList.remove('hidden');
