...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sales_method = 'Ticket Counter'; // Fixed for Counter Seller page
    ...
    if ($valid) {
        ...
        if ($check_stmt->fetch()) {
            $valid = false;
            $message = "<div style='display:flex; align-items:center; gap:10px;'><span style='font-size:1.5rem;'>⚠️</span> <div><strong>Duplicate Seller Code!</strong><br>A seller with the code '<strong>$seller_code</strong>' already exists in the system. Please use a unique code.</div></div>";
            $status = 'error';
        }
    }
    ...
    if ($valid) {
        try {
            ...
            $approvals = $_POST['approvals'] ?? [];
            $approval_years = $_POST['approval_years'] ?? [];
            $approval_data = [
                'selected' => $approvals,
                'years' => $approval_years,
                'other_comment' => $_POST['approval_other'] ?? '',
                'no_approval_note' => $_POST['no_approval_note'] ?? ''
            ];
            $approvals_json = json_encode($approval_data);
            
            $opening_hours = '';
            if (!empty($_POST['open_h']) && !empty($_POST['open_m']) && !empty($_POST['close_h']) && !empty($_POST['close_m'])) {
            ...
            $stmt->execute([
                ...
                $_POST['joined_year'] ?? '',
                $counter_state, $board_comment, $opening_hours,
                $image_paths['seller'],
            ...
            $message = "
                <div class='success-badge'>
                    ...
                    <div style='font-weight: 700; font-size: 1.2rem; margin-bottom: 5px;'>Counter Registration Successful!</div>
            ...
            $status  = 'success';
            log_activity($pdo, "Added Counter Seller", "Reg: $reg_number, Name: $seller_name", "seller");
        ...
        foreach (['front','side','inside','rear','seller'] as $key) {
            ...
            if (isset($_FILES['ie_image_'.$key]) && $_FILES['ie_image_'.$key]['error'] === UPLOAD_ERR_OK) {
                ...
                if (is_allowed_file($orig, $tmp)) {
                    ...
                    if (compress_image($tmp, $upload_dir . $fname, 60, 1000)) {
...
<head>
    ...
    <title>Counter Seller Entry - NLB Seller Map Portal</title>
    ...
    <style>
        ...
        .preview-label { color: var(--text-muted); font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.5px; flex-shrink: 0; padding-top: 2px; }
        .preview-val { color: var(--text-main); font-weight: 600; text-align: right; word-break: break-word; line-height: 1.4; }
        .preview-val.empty { color: rgba(255,255,255,0.15); font-weight: 400; font-style: italic; }
...
<div class="container wide">
    ...
    <div class="card" style="margin-bottom: 2rem; border: 2px solid var(--secondary-color); padding: 1.5rem;">
        ...
        <form method="POST" action="index.php?search_seller=<?php echo urlencode($edit_record['seller_code']); ?>" enctype="multipart/form-data">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem;">
                ...
                <input type="hidden" name="ie_sales_method" value="Ticket Counter">
                <div class="form-group" style="margin:0;">
    ...
    <div class="entry-layout">
        ...
        <div class="form-panel">
            <h2 style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                <span>🏪 Counter Seller Details Entry</span>
            ...
            <form action="" method="POST" enctype="multipart/form-data" id="entryForm">
                ...
                <div class="form-group" style="margin-bottom: 1.5rem; margin-top: 1rem;">
                    ...
                    <div class="radio-group" style="margin-top: 5px;">
                        ...
                        <input type="radio" id="bt_dlb" name="counter_state" value="DLB" onchange="toggleBoardOther(); updatePreview()">
                        <label for="bt_dlb" class="radio-pill"><span>DLB</span></label>
                        
                        <input type="radio" id="bt_private" name="counter_state" value="Private" onchange="toggleBoardOther(); updatePreview()">
                        <label for="bt_private" class="radio-pill"><span>Private</span></label>
                        
                        <input type="radio" id="bt_other" name="counter_state" value="Other" onchange="toggleBoardOther(); updatePreview()">
                        <label for="bt_other" class="radio-pill"><span>Other</span></label>
                    </div>
                </div>
                
                <div id="board_comment_wrap" class="form-group" style="display:none; margin-bottom: 1.5rem;">
                    <label for="board_comment">📝Comment</label>
                    <input type="text" id="board_comment" name="board_comment" placeholder="Specify other details" oninput="updatePreview()">
                </div>
                ...
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    <label for="address">🏠 Counter Address (ලිපිනය) *</label>
                    <textarea id="address" name="address" required placeholder="Enter Full Counter Address" oninput="updatePreview()"></textarea>
                </div>

                <!-- Optional: Home Address (Counter Sale only) -->
                <div class="form-group" style="margin-bottom: 1.5rem;">
                ...
                <div style="background: rgba(0,212,255,0.03); padding: 1.5rem; border-radius: 18px; border: 1px solid rgba(0,212,255,0.1); margin-bottom: 2rem;">
                    ...
                    <div class="field-row">
                        ...
                        <div class="form-group">
                            ...
                            <select id="gn_division" name="gn_division" required onchange="updatePreview()" disabled class="form-select-modern">
                    ...
                    <div class="form-group" style="margin-top: 1rem;">
                        ...
                        <div style="display: flex; gap: 10px;">
                            ...
                            <button type="button" onclick="getLocationLink()" class="btn-gps">📍 Get GPS</button>
                        </div>
                ...
                <div style="background: rgba(168,85,247,0.03); padding: 1.5rem; border-radius: 18px; border: 1px solid rgba(168,85,247,0.1); margin-bottom: 2rem;">
                    <h3 style="font-size: 0.95rem; color: #a855f7; margin-bottom: 1.25rem; display: flex; align-items: center; gap: 8px;">
                        <span>📜</span> Approvals (අනුමැතිය)
                    </h3>
                    
                    <div style="display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 1.5rem;">
                        <?php foreach(['LGA', 'RDA', 'UDA', 'NTC', 'CGR', 'Other'] as $app): ?>
                            <label class="approval-chip">
                                <input type="checkbox" name="approvals[]" value="<?php echo $app; ?>" onchange="toggleApprovalYear('<?php echo $app; ?>'); updatePreview()">
                                <span><?php echo $app; ?></span>
                            </label>
                        <?php endforeach; ?>
                    </div>

                    <div id="approval_years_section" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 1rem;">
                        <!-- Dynamic Renewal Year Selectors -->
                    </div>

                    <div id="approval_other_wrap" style="display: none; margin-bottom: 1rem;">
                        <label style="font-size: 0.85rem; color: var(--text-muted);">Specify Other Details</label>
                        <input type="text" id="approval_other" name="approval_other" placeholder="e.g. Local Authority Name" oninput="updatePreview()">
                    </div>

                    <div class="form-group">
                        <label style="font-size: 0.85rem; color: var(--text-muted);">IF No - Pls mention relative Authorities</label>
                        <textarea id="no_approval_note" name="no_approval_note" placeholder="Comment here if no standard approvals..." oninput="updatePreview()"></textarea>
                    </div>
                </div>

                <?php if (!empty($main_fields)): ?>
                <div style="margin-top:0.5rem; padding-top:1rem; border-top:1px solid var(--glass-border); margin-bottom: 2rem;">
                    <?php render_cf_group($main_fields); ?>
                </div>
                <?php endif; ?>

                <input type="hidden" name="sales_method" value="Ticket Counter">

                <div class="field-row" style="margin-bottom: 2rem; margin-top: 1rem;">
                    <div class="form-group">
                        <label>🕒 Opening Time (HH:MM)</label>
                        <div style="display: flex; gap: 8px; align-items: center;">
                            <select id="open_h" name="open_h" onchange="updatePreview()" class="form-select-modern" style="padding: 0.6rem; flex: 1;">
                                <option value="">HH</option>
                                <?php for($i=0; $i<24; $i++) { $h=str_pad($i,2,'0',STR_PAD_LEFT); echo "<option value='$h'>$h</option>"; } ?>
                            </select>
                            <span style="color: var(--text-muted);">:</span>
                            <select id="open_m" name="open_m" onchange="updatePreview()" class="form-select-modern" style="padding: 0.6rem; flex: 1;">
                                <option value="">MM</option>
                                <?php for($i=0; $i<60; $i+=5) { $m=str_pad($i,2,'0',STR_PAD_LEFT); echo "<option value='$m'>$m</option>"; } ?>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 8px; align-items: center;">
                            <select id="close_h" name="close_h" onchange="updatePreview()" class="form-select-modern" style="padding: 0.6rem; flex: 1;">
                                <option value="">HH</option>
                                <?php for($i=0; $i<24; $i++) { $h=str_pad($i,2,'0',STR_PAD_LEFT); echo "<option value='$h'>$h</option>"; } ?>
                            </select>
                            <span style="color: var(--text-muted);">:</span>
                            <select id="close_m" name="close_m" onchange="updatePreview()" class="form-select-modern" style="padding: 0.6rem; flex: 1;">
                                <option value="">MM</option>
                                <?php for($i=0; $i<60; $i+=5) { $m=str_pad($i,2,'0',STR_PAD_LEFT); echo "<option value='$m'>$m</option>"; } ?>
                            </select>
                        </div>
                ...
                <div class="form-group">
                    ...
                    <div class="upload-list">
                        ...
                        <div class="upload-item" onclick="document.getElementById('image_front').click()">
                            <div class="upload-info">
                                <div class="upload-title">Front view of the counter *</div>
                                <div class="upload-subtitle">(ඉදිරිපස දර්ශනය - click to select)</div>
                            </div>
                        ...
                        <div class="upload-item" onclick="document.getElementById('image_side').click()">
                            <div class="upload-info">
                                <div class="upload-title">Side view of the counter *</div>
                                <div class="upload-subtitle">(පැති දර්ශනය - click to select)</div>
                            </div>
                            <span class="upload-status-icon" id="icon_side">📸</span>
                            <input type="file" id="image_side" name="image_side" accept="image/*" required onchange="updateUI('side')">
                        </div>
                        <div class="upload-item" onclick="document.getElementById('image_inside').click()">
                            <div class="upload-info">
                                <div class="upload-title">Inside view of the counter *</div>
                                <div class="upload-subtitle">(ඇතුළත දර්ශනය - click to select)</div>
                            </div>
        ...
        <div class="info-panel">
            <div class="info-card" id="previewPanel">
                ...
                <div id="pdf-content" style="padding: 10px;">
                    ...
                    <div class="preview-section-title">🏢 Network Details</div>
    ...
    function submitAsDraft() {
        ...
        form.submit();
                    ...
                    <div class="preview-row">
                        <span class="preview-label">Reg Number:</span>
                    ...
                    <div class="preview-row">
                        ...
                        <span class="preview-val" id="prev_phone">-</span>
                    </div>

                    <div class="preview-section-title">🏠 Business Details</div>
                    <div class="preview-row">
                        <span class="preview-label">Counter Address:</span>
                        <span class="preview-val" id="prev_address">-</span>
                    </div>
                    <div class="preview-row" id="prev_address2_row" style="display:none;">
                        <span class="preview-label">Home Address:</span>
                        <span class="preview-val" id="prev_address2">-</span>
                    </div>
                    <div class="preview-row">
                        <span class="preview-label">Board Type:</span>
                        <span class="preview-val" id="prev_board">-</span>
                    </div>
                    ...
                    <div class="preview-row">
                        <span class="preview-label">Maps Link:</span>
                        <span class="preview-val" id="prev_location">-</span>
                    </div>

                    <div id="prev_approvals_container"></div>
                    ...
                    <div style="margin-top: 10px; display: grid; grid-template-columns: repeat(2, 1fr); gap: 5px;">
                        <div style="text-align:center;"><small>Seller</small><img id="thumb_seller" style="width: 100%; height: 60px; object-fit: cover; border-radius: 5px; display: none;"></div>
...
<script>
    function toggleBoardOther() {
        const boardOther = document.getElementById('bt_other').checked;
        const wrap = document.getElementById('board_comment_wrap');
        wrap.style.display = boardOther ? 'block' : 'none';
        if (boardOther) document.getElementById('board_comment').focus();
    }
    ...
    function toggleAddress2() {
        ...
        if (wrap.style.display === 'none') {
            ...
            btn.textContent = '− Remove Home Address';
            btn.style.background = 'rgba(239,68,68,0.1)';
            btn.style.borderColor = 'rgba(239,68,68,0.3)';
            btn.style.color = '#f87171';
        } else {
            wrap.style.display = 'none';
            btn.textContent = '+ Add Home Address';
    ...
    function toggleApprovalYear(app) {
        const container = document.getElementById('approval_years_section');
        const checkbox = document.querySelector(`input[name="approvals[]"][value="${app}"]`);
        if (!checkbox) return;
        const isChecked = checkbox.checked;
        
        if (app === 'Other') {
            document.getElementById('approval_other_wrap').style.display = isChecked ? 'block' : 'none';
        }

        if (isChecked) {
            if (!document.getElementById('year_wrap_' + app)) {
                const wrap = document.createElement('div');
                wrap.id = 'year_wrap_' + app;
                wrap.className = 'form-group';
                
                let options = '<option value="">-- Select Year --</option>';
                const curr = new Date().getFullYear();
                for (let y = curr; y >= 2015; y--) {
                    options += `<option value="${y}">${y}</option>`;
                }

                wrap.innerHTML = `
                    <label style="font-size:0.75rem; color:var(--text-muted); display:block; margin-bottom:4px;">${app} - Renewal Year</label>
                    <select name="approval_years[${app}]" class="form-select-modern" onchange="updatePreview()" style="padding: 0.5rem 0.8rem; font-size: 0.85rem;">
                        ${options}
                    </select>
                `;
            ...
            const wrap = document.getElementById('year_wrap_' + app);
            if (wrap) wrap.remove();
        }
    }
    ...
    function updatePreview() {
        ...
        if (addr2Value.trim()) {
            addr2Row.style.display = 'flex';
            document.getElementById('prev_address2').innerText = addr2Value;
        } else {
            addr2Row.style.display = 'none';
        }

        // Method
        const method = document.querySelector('input[name="sales_method"]:checked');
        if (method) document.getElementById('prev_method').textContent = method.value;

        // Board
        const board = document.querySelector('input[name="counter_state"]:checked');
        const boardComment = document.getElementById('board_comment').value;
        let boardText = board ? board.value : '-';
        if (board && board.value === 'Other' && boardComment) {
            boardText += ' (' + boardComment + ')';
        }
        set('prev_board', boardText);

        // Opening Hours
        const oh = document.getElementById('open_h').value;
        const om = document.getElementById('open_m').value;
        const ch = document.getElementById('close_h').value;
        const cm = document.getElementById('close_m').value;
        set('prev_hours', (oh && om && ch && cm) ? `${oh}:${om} to ${ch}:${cm}` : '');

        // Approvals Live Preview
        const appPreviewContainer = document.getElementById('prev_approvals_container');
        appPreviewContainer.innerHTML = '';
        const selectedApps = Array.from(document.querySelectorAll('input[name="approvals[]"]:checked')).map(cb => cb.value);
        
        if (selectedApps.length > 0) {
            const title = document.createElement('div');
            title.className = 'preview-section-title';
            title.innerHTML = '📜 Approvals Info';
            appPreviewContainer.appendChild(title);
            
            selectedApps.forEach(app => {
                const yearSelect = document.querySelector(`select[name="approval_years[${app}]"]`);
                const year = yearSelect ? yearSelect.value : '';
                const row = document.createElement('div');
                row.className = 'preview-row';
                let label = app;
                if (app === 'Other') {
                    const otherVal = document.getElementById('approval_other').value;
                    if (otherVal) label += ` (${otherVal})`;
                }
                row.innerHTML = `<span class="preview-label">${label}:</span><span class="preview-val">${year || 'Year not selected'}</span>`;
                appPreviewContainer.appendChild(row);
            });
        }
        
        const noAppName = document.getElementById('no_approval_note').value;
        if (noAppName.trim()) {
            const row = document.createElement('div');
            row.className = 'preview-row';
            row.innerHTML = `<span class="preview-label">Auth Note:</span><span class="preview-val">${noAppName}</span>`;
            appPreviewContainer.appendChild(row);
        }
    ...
    function shareWhatsApp() {
        ...
        const text = `*New Counter Registration*\n\n` +
                     `*Dealer:* ${d_code}\n` +
                     `*Agent:* ${a_code}\n` +
                     `*Counter:* ${s_code}\n` +
                     `*Name:* ${s_name}\n` +
    ...
    function takeSnapshot() {
        ...
        // Mirror the image for the canvas too if it's the front camera
        // But usually environment camera shouldn't be mirrored. 
        // For simplicity, we just draw what's on video.
        ...
        canvas.toBlob((blob) => {
            ...
            // Trigger the change event to update UI/preview
